...
namespace UZ.PDF.Font {
    ...
    //class CMapFont : DocumentFont {
         ...
         */
        public CMapAwareDocumentFont(PdfIndirectReference refFont) : base(refFont) {
            fontDic = (PdfDictionary)refFont.Target;
            InitFont();
        }
        ...
        private void ProcessToUnicode(){
            ...
            if (fontDic.ContainsKey("ToUnicode"))
                toUni = fontDic.Get("ToUnicode");
            if (toUni != null && toUni is PdfStream) {
                ...
                try {
                    ...
                    String enc = fontDic.Get("Encoding").Target.Value;
                    if(!enc.Equals("Identity-H"))
                        return;
                    PdfArray df = (PdfArray)fontDic.Get("DescendantFonts").Target;
                    PdfDictionary cidft = (PdfDictionary)df.Objects[0].Target;
                    

                    if (!cidft.ContainsKey("CIDSystemInfo"))
                    {
                        return;
                    }
                    PdfDictionary cidinfo = (PdfDictionary)cidft.Get("CIDSystemInfo").Target;

                    
                    if (!cidft.ContainsKey("Ordering"))
                    {
                        return;
                    }
                    PdfString ordering = (PdfString)cidinfo.Get("Ordering").Target;
