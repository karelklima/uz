...
namespace UZ.PDF.Font.Cmaps {
    ...
    class CMapParserEx {
        ...
        private static void ParseCid(String cmapName, AbstractCMap cmap, ICidLocation location, int level) {
            if (level >= MAXLEVEL)
                return;
            Tokenizer inp = location.GetLocation(cmapName);
            try {
                List<PdfObject> list = new List<PdfObject>();
                ContentParser cp = new ContentParser(inp);
                int maxExc = 50;
                while (true) {
                    try {
                        cp.ParseOperands(list);
                    }
                    catch {
                        if (--maxExc < 0)
                            break;
