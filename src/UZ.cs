...
namespace UZ
{
    class UZ
    {
        ...
        static void Debug()
        {
            ...
            //Index.IndexujVsechnyPredpisy(1990, 1992);

            /*try
            {
                TestInterpretace.MnohoNadpisuZaSebou(1990, 2013);
            }
            catch (UZException e)
            {
                Console.WriteLine(e.Message);
            ...
            //Text t = new Predpis(58, 2000).Text;

            //Console.Write(Sazec.VysazejText(t));

            try
            {
                Index.InterpretujPredpisyZObsahuCastek(2006, 2006, true);
            }
            catch (UZException e)
            {
                Console.WriteLine(e.Message);
            ...
            //Index.VygenerujUplnaZneni(2009, 20013);

            //Text t = Text.FromXmlFile(@"C:\Users\Karel\Documents\Visual Studio 2012\Projects\UZ\Sbirka\Sbirka\predpisy\2002\0076\pr0076-2002_0069-2013.xml");

            //int x = 1;

            /*Index.SmazCacheCastekRocnik(2010);
            Index.SmazCacheCastekRocnik(2011);
            Index.SmazCacheCastekRocnik(2012);
            Index.SmazCacheCastekRocnik(2013);*/
            //Index.ParsujObsahCastekZPdf(1990, 1992, true);

            /*Castka castka = new Castka(31, 2010);
            File.Delete(castka.SouborText);
            File.Delete(castka.SouborInfo);
            List<Predpis> predpisy = castka.Predpisy;
            Console.WriteLine("ok");*/

            /*
            // PDF DEBUG
            string SouborPdf = Nastaveni.PracovniAdresar.FullName + @"\Test\sb0001-1998.pdf";
            //string SouborPdf = Nastaveni.PracovniAdresar.FullName + @"\Test\sb0003-1992.pdf";
            float spaceWidthFactor = 1f;
            float lineBreakFactor = 1f;



            try
            {
                StructuredDocument doc = StructuredDocument.FromPdfFile(SouborPdf, 0, spaceWidthFactor, lineBreakFactor);

                doc.ToXmlFile("Test/structureddocument.txt");

                //StructuredDocument docCopy = StructuredDocument.FromXmlFile("Test/structureddocument.txt");
                //docCopy.ToXmlFile("Test/structureddocumentcopy.txt");
            }
            catch (KeyNotFoundException e) { }
            /*catch (PdfException pdfe)
            {
                Console.Write(pdfe.Message);
            }*/
